...
```
Runs automatically in `adopt.sh` Phase C. This is the ranking + display layer — the actual `npx skills add` invocation is still left to the user. GitHub-query-based discovery for `jarvis find` is planned for v0.3.
...
*(Adopt mode in v0.2.0; bootstrap reached parity in v0.2.1 — the greenfield installer now writes the same `wiki-ownership` / `wiki-location` / `owned-files` state that adopt does.)*
...
- **Auto-apply archetype overlay on confident detection** (≥3 stack-tags) — with escape hatches (`--no-archetype`, interactive Y/n/never, `archetype-overlay: never` in preferences). The `web-app` and `telegram-bot` overlays ship themselves instead of waiting for `--enable` (llm-agent overlay completes in v0.2.1 — see [release notes](RELEASE_NOTES_v0.2.1.md)).
